...
--   Student ID: 6288089
--   Fullname: Intr-orn Lertsupakitsin
--   Section: 1
------------------------------------------------------------- */
...
-- q3
SELECT COUNT(CRS_CREDIT) AS '#_of_courses'
FROM course
...
WHERE EMP_FNAME LIKE 'P%' OR EMP_FNAME LIKE 'A%'
...
-- 26
SELECT EMP_NUM, LOWER(CONCAT(EMP_FNAME,'.', LEFT(EMP_LNAME, 3), '@tinycollege.edu')) as email
FROM professor
WHERE DEPT_CODE = 'ACCT';

SELECT STU_DOB FROM student;
...
-- 29  
SELECT EMP_FNAME, EMP_LNAME, 2020-YEAR(EMP_DOB) as age
FROM professor
WHERE 2020-YEAR(EMP_DOB) >75
ORDER BY 2020-YEAR(EMP_DOB) DESC;

-- 30
SELECT max(2020-YEAR(STU_DOB)) as age
FROM student;
...
-- 40
SELECT STU_FNAME, STU_LNAME, count(GRADE) as num_gradeA
from student s
JOIN enroll e on s.STU_NUM = e.STU_NUM
WHERE DEPT_CODE = 'CIS' and GRADE = 'A'
Group by STU_FNAME
...
-- 41
SELECT EMP_FNAME, EMP_LNAME, count(s.EMP_NUM) as num_advisee
from professor p
JOIN student s on p.EMP_NUM = s.EMP_NUM
GROUP BY EMP_FNAME
...
from class cl 
JOIN course c on cl.COURSE_CODE = c.COURSE_CODE
WHERE c.DEPT_CODE = 'CIS' and COURSE_NAME like 'Intro%' and CLASS_SECTION = '1';

-- 43
SELECT d.DEPT_CODE, DEPT_NAME, count(distinct(p.EMP_NUM)) as num_prof, count(distinct(STU_NUM)) as num_stu, round(count(distinct(STU_NUM))/count(distinct(p.EMP_NUM)),2) as stuprof_ratio
from department d
...
-- 44
SELECT d.DEPT_CODE, DEPT_NAME, CONCAT(p.EMP_FNAME, ' ', p.EMP_LNAME) as head_dept
from department d
JOIN professor p on d.EMP_NUM = p.EMP_NUM
group by d.DEPT_CODE
order by d.DEPT_CODE asc;

-- 45
SELECT s.STU_NUM, STU_FNAME, STU_LNAME, count(STU_CLASS) as total_class, sum(CREDIT) as total_credit 
from student s
join enroll e on s.STU_NUM = e.STU_NUM
group by s.STU_NUM
having count(STU_CLASS)>=6 or count(CREDIT)>=18
