...
-- 1
SELECT cl.CLASS_CODE as "class_code", c.COURSE_NAME as "course_name", p.EMP_FNAME as "emp_fname", p.EMP_LNAME as "emp_lname", COUNT(cl.CLASS_CODE) as "num_stu"
FROM class cl
...
WHERE cl.CLASS_SECTION = 3
GROUP BY cl.CLASS_CODE;

-- 2
SELECT cl.CLASS_CODE as "class_code", c.COURSE_NAME as "course_name"
FROM class cl
JOIN course c on cl.COURSE_CODE = c.COURSE_CODE
...
UNION
SELECT p.EMP_FNAME as "first name", p.EMP_LNAME as "last name", p.EMP_NUM as "ID"
FROM professor p
...
-- 6
SELECT s.STU_FNAME as "first name", s.STU_LNAME as "last name", s.STU_GPA as "stu_gpa"
FROM student s
WHERE s.STU_GPA >
	(SELECT AVG(STU_GPA)
    FROM student
    WHERE DEPT_CODE = "MATH")
ORDER BY s.STU_GPA DESC;
